...
function create_sysimage(packages::Union{Symbol, Vector{Symbol}}=Symbol[];
    ...
    o_init_file = julia_init_c_file == "" ? "" : compile_c_init_julia(julia_init_c_file, sysimage_path)
...
function create_sysimg_from_object_file(input_object::String, sysimage_path::String; o_init_file::String="")
    julia_libdir = dirname(Libdl.dlpath("libjulia"))

    o_files = [input_object]
    length(o_init_file) > 0 && push!(o_files, o_init_file)
